...
const float p[4]={39.7284, 20.8642, 2., 4.}, kk = 100000, dev=0.08 ;	// set S0.5
//float p[4]={56.45, 29.23, 0.8, 2.8}, kk = 1000000, dev=0.08 ; // set S0.2
const int noff=1 ; 
...
int main(int argn, char **argc)
{
  ...
  ofstream data(name) ;
	...
	for (int nnn=0;nnn<nsam;nnn++) {
	  ...
	  double tt=0, ttt=0, dt=1./(1<<10) ; ; // BW: 1<<9 normally if not indicated in file name
	  for (i=0;;i++) {
	...
	//    }
	    ...
	    if (outtype==0) {
			...
			} else if (outtype==1 && tt>0.2*tmax && i%16==0) { // BW: normally  128
			  data<<tt<<"\t"<<nA<<"\t"<<nB<<endl ; 
			}
	  }  
	}
